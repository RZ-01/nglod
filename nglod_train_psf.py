...
def initialize_nglod_model(device):
...
def psf_deconvolution_training_step(model: nn.Module, norm_volume_np: np.ndarray,
    ...
    writer.add_scalar("PSF_Deconv/Total_Loss_MSE_3D", total_loss.item(), global_step)
    
    return total_loss


def save_checkpoint(model, nglod_args, save_path):
    save_obj = {
...
def main():
    parser = argparse.ArgumentParser(description="Train NGLOD for PSF deconvolution from scratch")
    parser.add_argument("--volume_tif", type=str, default="Mouse_Heart_Angle0_patch.tif")
    ...
    parser.add_argument("--save_path", type=str, default="checkpoints/nglod_psf_angle_0.pth")
    parser.add_argument("--logdir", type=str, default="runs/psf_deconv_nglod_train")
    ...
    model, nglod_args = initialize_nglod_model(device)
    ...
    pbar = tqdm(range(args.steps), desc="PSF Deconv Training")
    for step in pbar:
        optimizer.zero_grad(set_to_none=True)
        
        total_loss = psf_deconvolution_training_step(
            model=model,
        ...
        if (step + 1) % 10 == 0:
            pbar.set_postfix(loss=f"{total_loss.item():.8f}")

    save_checkpoint(model, nglod_args, args.save_path)
