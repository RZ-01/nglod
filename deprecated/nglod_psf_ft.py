...
def sample_block_start_indices(volume_shape, block_shape, psf_shape=None, step=0):
    ...
    z_steps = (z0_high_inclusive - z0_low) // (bz // 4)  # Z方向步长
    y_steps = y_max // (by // 4) # Y方向步长  
    x_steps = x_max // (bx // 4)  # X方向步长
...
def psf_finetune_step(model: nn.Module, norm_volume_np: np.ndarray,
                      ...
                      psf_kernels: torch.Tensor, block_shape, block_coords=None, reg_lambda=0.01,
                      random_points_lambda=0.1, num_random_points=1000) -> torch.Tensor:
    model.train()
    ...
    if len(random_coords) > 0:
        ...
        # 计算与目标值的L1损失（这里目标值设为0，鼓励块外区域保持稳定）
    ...
    writer.add_scalar("PSF_FT/Random_Points_Loss", random_points_loss.item() if isinstance(random_points_loss, torch.Tensor) else random_points_loss, global_step)
